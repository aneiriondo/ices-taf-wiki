...
In this example the aim is to convert the following simple code, taken from the help file from
the `trees` dataset (run `?trees` in R), into the TAF structure.
...
At each stage you can test with `taf.bootstrap()` and `sourceTAF()` as appropriate, and finally check
the whole thing runs by calling `taf.bootsrap(taf=TRUE)` and `sourceAll()`. 
...
run_dir <- download.analysis("ices-taf-dev/wiki-example-3")
x <- capture.output(run.analysis(run_dir))
mkdir("example-3")
cp(file.path(run_dir, "*"), "example-3")
```


```{r, echo = FALSE, results="asis"}
paths <- dir("example-3", full.names = TRUE, recursive = TRUE)
printDir(paths)
```
