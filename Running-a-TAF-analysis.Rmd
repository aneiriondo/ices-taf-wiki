...
working directory that your R sessions is running in (see `getwd()` to find out what this is).

```{r}
run_dir <- download.analysis("ices-taf/2019_san.sa.6", dir = ".")
```

which results in the following files the directory created by the above function:
